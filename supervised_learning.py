...
ammino_acid = list("ACDEFGHIKLMNPQRSTVWY"); amino_acid_to_index = {a:i for i,a in enumerate(AA)}
default = {
  "vdj_tcr":"cdr3","vdj_epi":"antigen.epitope","vdj_score":"vdjdb.score",
  "trait_tcr":"CDR3β","trait_epi":"Epitope","trait_bind":"Binding",
  "pos_score":3,"neg_scores":(1,2),"neg_ratio":1.0,"maxlen_tcr":30,"maxlen_epi":15
}
LABEL_MAP = {'binder':1,'binding':1,'binds':1,'yes':1,'true':1,'positive':1,'pos':1,'1':1,
             'nonbinder':0,'non-binding':0,'nonbinding':0,'no':0,'false':0,'negative':0,'neg':0,'0':0}

def load_any(path_or_url: str, sheet: Optional[str]=None) -> pd.DataFrame:
    p = path.lower()
...
def one_hot(seq: str, max_len: int) -> np.ndarray:
    m = np.zeros((max_len, len(AA)), dtype=np.float32)
    L = min(len(seq), max_len)
    for i in range(L): m[i, amino_acid_to_index[seq[i]]] = 1.0
    return m
...
def build_vdj_supervised(df: pd.DataFrame, tcr_col: str, epi_col: str, score_col: str,
    ...
    df[tcr_col] = df[tcr_col].map(clean_seq); df[epi_col] = df[epi_col].map(clean_seq)
    ...
    if pos.empty: 
      raise SystemExit("No positives (score==3) in VDJdb.")
    neg_pool = df[df[score_col].isin(neg_scores)][[tcr_col, epi_col]].drop_duplicates().reset_index(drop=True)
    if neg_pool.empty: 
      raise SystemExit("No negatives pool (score in {1,2}) in VDJdb.")
    ...
    neg = neg_pool[[tcr_col]].copy(); neg[epi_col] = shuffled_epi
    same = neg[epi_col].values == neg_pool[epi_col].values
    if same.any():
        idx = np.where(same)[0]; neg.loc[idx, epi_col] = np.roll(neg.loc[idx, epi_col].values, 1)
...
def build_trait_validation(df: pd.DataFrame, tcr_col: str, epi_col: str,
    ...
    if bind_col and bind_col in df.columns:
        y_try = to01(df[bind_col])
        if y_try.notna().any():
            keep = y_try.notna()
            X = X.loc[keep].reset_index(drop=True); y = y_try.loc[keep].astype(int).reset_index(drop=True)
    return X, y

def train_pipeline(maxlen_tcr:int, maxlen_epi:int) -> Pipeline:
    return Pipeline([
        ("fe",   lambda X: featurize_df(X, maxlen_tcr, maxlen_epi)),
...
class FeatWrapper(BaseEstimator, TransformerMixin):
    def __init__(self, maxlen_tcr:int, maxlen_epi:int):
        self.maxlen_tcr=maxlen_tcr; self.maxlen_epi=maxlen_epi
...
def train_vdjdb(vdj_path: str, sheet: Optional[str], tcr_col: str, epi_col: str, score_col: str,
    ...
    pipe = Pipeline([("fe", FeatWrapper(maxlen_tcr, maxlen_epi)),
                     ("sc", StandardScaler(with_mean=False)),
                     ("clf", LogisticRegression(penalty="l2", C=1.0, solver="liblinear",
                                                max_iter=2000, class_weight="balanced"))])
    ...
    metrics = {
        ...
        "repr":"onehot_lr","maxlen_tcr":maxlen_tcr,"maxlen_epi":maxlen_epi,
        "tcr_col":"tcr","epitope_col":"epitope","score_col":score_col,
        "pos_score":pos_score,"neg_scores":list(neg_scores),"neg_ratio":neg_ratio
    }
    joblib.dump({"pipeline": pipe, "tcr_col":"tcr", "epitope_col":"epitope", "metrics": metrics}, model_out)
    print(json.dumps(metrics, indent=2)); print(f"\nSaved model to: {model_out}")

def validate_trait(model_path: str, trait_path: str, sheet: Optional[str],
                   tcr_col: str, epi_col: str, bind_col: Optional[str]):
    bundle = joblib.load(model_path); pipe = bundle["pipeline"]
    df = load_any(trait_path, sheet)
    X, y = build_trait_validation(df, tcr_col, epi_col, bind_col)
    p = pipe.predict_proba(X)[:,1]; yhat = (p>=0.5).astype(int)
    if y is not None:
        metrics = {
            "external": {
            ...
            "n_val": int(len(X)), "tcr_col":"tcr","epitope_col":"epitope","label_col":bind_col
        }
    else:
        metrics = {
            "note":"No usable labels in TRAITdb dataset, binding might be missing or not able to be mapped. Reporting score summary.",
            "n_val": int(len(X)),
            "summary": {"mean_prob": float(np.mean(p)),"std_prob": float(np.std(p)),"pct_pred_positive@0.5": float(np.mean(yhat))}
        }
...
def main():
    ...
    tr.add_argument("--vdjdb_path", required=True); tr.add_argument("--vdjdb_sheet", default=None)
    tr.add_argument("--tcr_col", default=DEF["vdj_tcr"]); tr.add_argument("--epitope_col", default=DEF["vdj_epi"])
    tr.add_argument("--score_col", default=DEF["vdj_score"])
    tr.add_argument("--pos_score", type=int, default=DEF["pos_score"])
    tr.add_argument("--neg_scores", type=int, nargs="+", default=list(DEF["neg_scores"]))
    tr.add_argument("--neg_ratio", type=float, default=DEF["neg_ratio"])
    tr.add_argument("--maxlen_tcr", type=int, default=DEF["maxlen_tcr"])
    tr.add_argument("--maxlen_epi", type=int, default=DEF["maxlen_epi"])
    ...
    va.add_argument("--trait_path", required=True); va.add_argument("--trait_sheet", default=None)
    va.add_argument("--tcr_col", default=DEF["trait_tcr"]); va.add_argument("--epitope_col", default=DEF["trait_epi"])
    va.add_argument("--bind_col", default=DEF["trait_bind"])
